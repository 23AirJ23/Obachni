(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        let normLink = currentLink.slice(1);
        let locationList = document.querySelectorAll(".menu-nav__link");
        locationList.forEach((element => {
            let current = element.getAttribute("href");
            if (normLink === current) element.classList.add("active");
        }));
        let lines = document.querySelectorAll(".item-agent");
        lines.forEach((element => {
            element.addEventListener("click", (function(e) {
                lines.forEach((el => {
                    el.classList.remove("_choosen");
                }));
                element.classList.add("_choosen");
            }));
        }));
        window.onscroll = function() {
            let autorize = location.pathname.slice(1);
            console.log(autorize);
            if (autorize !== "autorize.html") scrollFunction();
        };
